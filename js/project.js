QComment.controller('Project', ['$rootScope', '$scope', 'projectDefaults', 'API'
    ...
    $scope.price = jQuery(document).find("input[ng-model='price']").val();
    if ($scope.app_key.length > 0) {
        ...
        $scope.createProject = function() {
            ...
            if($scope.title == ''){
                $scope.title = jQuery('.wrap').find("input[ng-model='title']").val();
            }
            if($scope.price == ''){
                $scope.price = jQuery('.wrap').find("input[ng-model='price']").val();
            }
            var data = {
                ...
                cnt_day_limit: $scope.cnt_day_limit,
                limit: $scope.limit,
                min_day_limit: $scope.min_day_limit,
                max_day_limit: $scope.max_day_limit,
                delay_from: $scope.delay_from,
                delay_to: $scope.delay_to,
...
jQuery( "#qcomment_tariff_id" )
    .change(function () {
        var select_price = jQuery(this).find('option:selected').data("selectprice");
        jQuery(this).closest('table').find("input[ng-model='price']").val(select_price.substr(0, select_price.length - 4));
    });

jQuery('.wrap')
 .on('keyup', "input[name='post_title']", function(){
        var me = jQuery('.wrap');
        me.find("input[ng-model='title']").val(jQuery(this).val());
    });
jQuery('.wrap')
    .on('change', "input[name='post_title']", function(){
        var me = jQuery('.wrap');
        me.find("input[ng-model='title']").val(jQuery(this).val());
    });