    ...
    public function save_postdata() {
        ...
        if ( empty( $project_id ) && 1 === (int)get_option( 'qc_buy_for_all' ) ) {
            ...
            if ( empty( $qc_tariff_id ) ) {
                ...
                $requestData[ 'tariff_id' ] = $qc_tariff_id;
            }
            ...
            $qc_cnt_day_limit = get_option( 'qc_cnt_day_limit' );
            if ( !empty( $qc_cnt_day_limit ) ) {
                $requestData[ 'cnt_day_limit' ] = $qc_cnt_day_limit;
            }
            ...
            $qc_delay_from = get_option( 'qc_delay_from' );
            if ( !empty( $qc_delay_from ) ) {
                $requestData[ 'delay_from' ] = $qc_delay_from;
            }

            $qc_delay_to = get_option( 'qc_delay_to' );
            if ( !empty( $qc_delay_to ) ) {
                $requestData[ 'delay_to' ] = $qc_delay_to;
            }
            ...
            $qc_limit_url = get_option( 'qc_limit_url' );
            if ( !empty( $qc_limit_url ) ) {
                $requestData[ 'limit_url' ] = $qc_limit_url;
            }

            $qc_limit_url_day = get_option( 'qc_limit_url_day' );
            if ( !empty( $qc_limit_url_day ) ) {
                $requestData[ 'limit_url_day' ] = $qc_limit_url_day;
            }
            ...
            $response = wp_remote_post( 'https://qcomment.ru/api/newproject', array(
                    'method' => 'POST',
                    'timeout' => 45,
                    'redirection' => 5,
                    'httpversion' => '1.0',
...
//        wp_enqueue_style( 'qcomment_main', QC_URL . '/css/main.css', array(), '1.0' );
	    ...
	    $data[ 'subject' ] = empty($data[ 'subject' ]) ? 0 : $data[ 'subject' ];

       // if ( false === ( $data[ 'tariff' ] = get_transient( 'tariff' ) ) ) {
//            $resp = wp_remote_get( 'https://qcomment.ru/api/getTariff' );
//            $data[ 'tariff' ] = json_decode($resp['body'],true);
	        $data[ 'tariff' ] = $qcomment_data['tariff'];

			set_transient( 'tariff',$data[ 'tariff' ], 24 * HOUR_IN_SECONDS );
        //}

        $data[ 'rates' ] = $data[ 'tariff' ][ 0 ][ 'id' ];




        $data[ 'tariff_id' ] = qc_get_value( $data[ 'post_id' ], 'tariff_id' );
        $data[ 'price' ] = qc_get_value( $data[ 'post_id' ], 'price' );
        ...
        $data[ 'cnt_day_limit' ] = qc_get_value( $data[ 'post_id' ], 'cnt_day_limit' );
        $data[ 'min_day_limit' ] = qc_get_value( $data[ 'post_id' ], 'min_day_limit' );
        $data[ 'max_day_limit' ] = qc_get_value( $data[ 'post_id' ], 'max_day_limit' );
        $data[ 'delay_from' ] = qc_get_value( $data[ 'post_id' ], 'delay_from' );
        $data[ 'delay_to' ] = qc_get_value( $data[ 'post_id' ], 'delay_to' );
        $data[ 'limit_hour' ] = qc_get_value( $data[ 'post_id' ], 'limit_hour' );

        $data[ 'limit_url' ] = qc_get_value( $data[ 'post_id' ], 'limit_url' );
        $data[ 'limit_url_day' ] = qc_get_value( $data[ 'post_id' ], 'limit_url_day' );
        ...
        $balance_rub = $this->balance_account();

	     $data[ 'balance' ] = array(
			'rub' => $balance_rub->balance,
	    );
	...
	public function balance_account($currency_type = 'RUB', $api = ''){
		$appkey = empty($api) ? get_option( 'qc_app_key' ) : $api;
		if(!empty($appkey)) {
			$requestData = array(
				'apicode'  => $appkey,
			...
			$response = wp_remote_post( 'https://qcomment.ru/api/balance', array(
					'method'      => 'POST',
					'timeout'     => 45,
					'redirection' => 5,
					'httpversion' => '1.0',
			...
			return array(
				'status' => 'no',
				'reason' => 'Не указан App Key приложения',
			);
		}
	}
}
